"""
Command-Line Interface for PQC-WireGuard: Hybrid Post-Quantum Tunneling & Key Negotiation Supervisor.
"""
import argparse
import csv
import sys
...
from .agents import PQCWireguardCoordinator

coordinator = PQCWireguardCoordinator()
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="post-quantum-vpn-wireguard-agent", description="PQC-WireGuard: Hybrid Post-Quantum Tunneling & Key Negotiation Supervisor")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  PQC-WIREGUARD: HYBRID POST-QUANTUM TUNNELING & KEY NEGOTIATION SUPERVISOR")
        print(f"  Domain: Post-Quantum Cryptography & Zero-Knowledge | Standard: IETF Noise Protocol & PQC Hybrid Extension")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting PQC-WireGuard: Hybrid Post-Quantum Tunneling & Key Negotiation Supervisor on http://{args.host}:{args.port}")
